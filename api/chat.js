  ...
  try {
    ...
    const tools = [
      ...
      },
      {
        ...
        input_schema: { type: 'object', properties: { to: { type: 'string', description: 'Recipient email(s), comma separated' }, subject: { type: 'string', description: 'Email subject' }, body: { type: 'string', description: 'Email body (plain text)' }, cc: { type: 'string', description: 'CC recipients, comma separated' }, bcc: { type: 'string', description: 'BCC recipients, comma separated' } }, required: ['to', 'subject', 'body'] }
      },
      {
        name: 'outlook_send_draft',
        description: 'Send an Outlook email draft. Only after user confirms.',
        input_schema: { type: 'object', properties: { draftId: { type: 'string', description: 'Draft ID from outlook_create_draft' } }, required: ['draftId'] }
      },
      {
        name: 'outlook_send_direct',
        description: 'Send an Outlook email directly. Only use when user explicitly requests immediate send.',
        input_schema: { type: 'object', properties: { to: { type: 'string', description: 'Recipient email(s), comma separated' }, subject: { type: 'string', description: 'Email subject' }, body: { type: 'string', description: 'Email body (plain text)' }, cc: { type: 'string', description: 'CC recipients' }, bcc: { type: 'string', description: 'BCC recipients' } }, required: ['to', 'subject', 'body'] }
      },
      // ===== SLACK TOOLS =====
      {
        name: 'slack_list_channels',
        description: 'List all Slack channels and group DMs the user is a member of.',
        input_schema: { type: 'object', properties: {} }
      },
      {
        name: 'slack_list_members',
        description: 'List all members of a Slack workspace.',
        input_schema: { type: 'object', properties: {} }
      },
      {
        name: 'slack_send_message',
        description: 'Send a message to a Slack channel or DM.',
        input_schema: { type: 'object', properties: { channelId: { type: 'string', description: 'Channel ID or user ID (starts with C or U)' }, text: { type: 'string', description: 'Message text (plain text only)' } }, required: ['channelId', 'text'] }
      },
      {
        name: 'slack_create_reminder',
        description: 'Create a Slack reminder to notify the user at a specific time.',
        input_schema: { type: 'object', properties: { text: { type: 'string', description: 'Reminder text' }, time: { type: 'string', description: 'When to remind (e.g. "in 5 minutes", "at 3pm", "tomorrow at 10am")' } }, required: ['text', 'time'] }
      },
    ];
nput_schema: { type: 'object', properties: { to: { type: 'string', description: 'Recipient email(s), comma separated' }, subject: { type: 'string', description: 'Email subject' }, body: { type: 'string', description: 'Email body' }, cc: { type: 'string', description: 'CC recipients' }, bodyType: { type: 'string', description: 'Text or HTML (default Text)' } }, required: ['to', 'subject', 'body'] }
      },
      {
      ...
      try {
        switch (name) {
          ...
          case 'gmail_get_attachments': {
            ...
            const attachments = [];    function findParts(parts) {
          ...
          case 'google_drive_search': {
            ...
            if(!r.ok){const e=await r.text();return {error:e};}
            const d = await r.json();
            return {files:(d.files||[]).map(f=>({id:f.id,name:f.name,type:f.mimeType,modified:f.modifiedTime,link:f.webViewLink||''}))}; n          }
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const parts = data.candidates[0].content.parts || [];
      const fcs = parts.filter(p => p.functionCall);
      if (fcs.length > 0 && (googleToken || msToken)) {
        currentContents.push({ role: 'model', parts: parts });
        const rps = [];
        for (const part of fcs) {
          const fc = part.functionCall;
