...
export default function ReportView({ skills, executionLogs, getSkillStats, getOverallStats }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "12px 24px", borderBottom: `1px solid ${V.border}`, background: V.sb }}>
        <div style={{ fontSize: 20, fontWeight: 700, color: V.t1 }}>📊 レポート</div>
        <div style={{ fontSize: 14, color: V.t3, marginTop: 2 }}>スキルのパフォーマンスと実行履歴</div>
      </div>
          ...
          <>
            ...
            <div style={{ background: V.card, borderRadius: 10, border: `1px solid ${V.border}` }}>
              <div style={{ padding: "14px 16px", borderBottom: `1px solid ${V.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 16, fontWeight: 600, color: V.t1 }}>実行ログ</span>
              ...
              {recentLogs.length === 0 ? (
                ...
                <div style={{ maxHeight: 400, overflowY: "auto" }}>
                  {recentLogs.map((log, i) => {
                    ...
                    return (
                      <div key={log.id || i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderBottom: i < recentLogs.length - 1 ? `1px solid ${V.border}` : "none", fontSize: 13 }}>
